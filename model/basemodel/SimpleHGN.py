...
class SimpleHGN(nn.Module):
    def __init__(self, config):
        ...
        in_dims = [config['num_features'] for _ in range(config['num_ntypes'])]
        out_dims = config['out_dim']
        num_etypes = config['num_etypes']
        num_hidden = config['hidden_dim']
        heads = config['num_heads']
        feat_drop = config['dropout']
        attn_drop = config['dropout']
        negative_slope = config['slope']
        ...
        self._fc_list = nn.ModuleList([nn.Linear(in_dim, num_hidden, bias=True) for in_dim in in_dims])


        for fc in self._fc_list:
            nn.init.xavier_normal_(fc.weight, gain=1.414)
        # input projection (no residual)
        self._gat_layers.append( myGATConv(edge_dim, num_etypes, num_hidden, num_hidden, heads[0], feat_drop, attn_drop, negative_slope, False, self.activation, alpha=alpha) )
        # hidden layers
        for l in range(1, self._num_layers):
            # due to multi-head, the in_dim = num_hidden * num_heads
            self._gat_layers.append( myGATConv(edge_dim, num_etypes, num_hidden * heads[l-1], num_hidden, heads[l], feat_drop, attn_drop, negative_slope, residual, self._activation, alpha=alpha) )
        
        # output projection
        self._gat_layers.append(myGATConv( edge_dim, num_etypes, num_hidden * heads[-2], out_dims, heads[-1], feat_drop, attn_drop, negative_slope, residual, None, alpha=alpha) )
    ...
    def forward(self, features_list, e_feat):
        ...
        for l in range(self._num_layers):
            h, res_attn = self._gat_layers[l](self.g, h, e_feat, res_attn=res_attn)
            h = h.flatten(1)
        # output projection
        logits, _ = self._gat_layers[-1](self.g, h, e_feat, res_attn=None)
