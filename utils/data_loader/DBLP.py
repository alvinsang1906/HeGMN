...
from cogdl.datasets.gtn_data import DBLP_GTNDataset
"""
    num_ntype: 4
        0
        1
...
class data_loader():
    ...
    def __init__(self, path=None):
        ...
        if all(os.path.basename(file) in _req_pkl_files for file in _pkl_files) and len(_pkl_files) == len(_req_pkl_files):
            ...
            _hetero_data = DBLP_GTNDataset('./datasets')
            _homoro_graph = self._torch2nx(_hetero_data)
            self.graphs, hetero_list = self._generate_graphs(700, _homoro_graph, _hetero_data.data)
            self.train_data, self.test_data = hetero_list[:560], hetero_list[560:]
    ...
    def _get_ged(self, path):
        ...
        """
        ged_hetero, ged_homoro = torch.full((700, 700), float('inf')), torch.full((700, 700), float('inf'))
        norm_ged_hetero, norm_ged_homoro = torch.full((700, 700), float('inf')), torch.full((700, 700), float('inf'))
        for i in range(560):
