...
from cogdl.datasets.gtn_data import ACM_GTNDataset
"""
    gtn-acm数据集(无向图)：
    8994个节点,12961条边
    节点三类：0 1 2
    边三类：
        ...
        (1, 0) (0, 1) : 1
        (0, 0) : 2
    1000张图，平均节点个数为9.039，平均边数为8.419
"""

class data_loader():
    r"""这是读取ACM数据集，并尝试使用BFS算法将其转换为可以进行相似度计算的小异质图"""
    def __init__(self, path=None):

        if path == None:
            _path = './datasets/gtn-acm/'
        else:
            _path = path
        ...
        if all(os.path.basename(file) in _req_pkl_files for file in _pkl_files) and len(_pkl_files) == len(_req_pkl_files):
            ...
            _hetero_data = ACM_GTNDataset('./datasets')
            _homoro_graph = self._torch2nx(_hetero_data)
            self.graphs, hetero_list = self._generate_graphs(1000, _homoro_graph, _hetero_data.data)
            self.train_data, self.test_data = hetero_list[:800], hetero_list[800:]
    ...
    def _generate_graphs(self, nums, homoro_data, hetero_data):
        ...
        while(len(graphs) < nums):
            ...
            max_num = random.randint(6, 8) # 最多包含的节点个数 [6, 8]
            self._BFS(g, homoro_data, start_node, max_num, 0.5)
            if g.number_of_nodes() < 5 or g.number_of_node_types < 4: # 节点个数太少 或者 没有包含全部的节点类型 就舍弃
                continue
            ...
            if exist_nid in exist_nid_list:
                continue
    ...
    def _get_ged(self, path):
        r"""
            ...
            参数 path: str, 数据集DBLP存储的目录
        """
        ged_hetero, ged_homoro = torch.full((1000, 1000), float('inf')), torch.full((1000, 1000), float('inf'))
        norm_ged_hetero, norm_ged_homoro = torch.full((1000, 1000), float('inf')), torch.full((1000, 1000), float('inf'))
        for i in range(800):
    ...
    def _save_files(self, _req_pkl_files):
        for pkl_file in _req_pkl_files:
            with open ('./datasets/gtn-dblp/' + pkl_file, 'wb') as f:
